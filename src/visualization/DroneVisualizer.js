...
export class DroneVisualizer {
  ...
  _initScene() {
    ...
    // グリッド
    const grid = new THREE.GridHelper(50, 50, 0x3d7a3d, 0x3d7a3d);
  ...
  _initDroneMesh() {
    ...
    const bodyMat = new THREE.MeshLambertMaterial({ color: 0x333355 });
    const body    = new THREE.Mesh(bodyGeo, bodyMat);
    body.castShadow = true;
    this.droneGroup.add(body);

    // ── アーム (+ コンフィグ) ──────────────────────────────────────────
    const armMat = new THREE.MeshLambertMaterial({ color: 0x555577 });
    const armPositions = [
      { x:  l, y: 0, z:  0 }, // 前 (+x)
      { x: -l, y: 0, z:  0 }, // 後 (-x)
    ...
    // ── モータ・プロペラ ───────────────────────────────────────────────
    const motorColors = [0xff4444, 0x44ff44, 0x4444ff, 0xffff44];
  ...
  _initTrajectory() {
    ...
    const mat = new THREE.LineBasicMaterial({ color: 0x00aaff, opacity: 0.6, transparent: true });
